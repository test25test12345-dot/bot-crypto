...
import {
  ...
  const logToFile = (message: string) => {
      const logFilePath = path.join("/usr/work/wallettrackalertbotcpmm/", 'logs', 'track.log');
...
export async function decodeRaydiumCpmmTxn(tx: VersionedTransactionResponse) {    
  if (!tx.meta || tx.meta?.err) return;

  const parsedIxs = IX_PARSER.parseTransactionWithInnerInstructions(tx);

  const programIxs = parsedIxs.filter((ix) =>
      ix.programId.equals(new PublicKey('CPMMoo8L3F4NbTegBCKVNunggL7H1ZpdTHKxQB5qKP1C')),
  );
...
//   logToFile(`result========== ${JSON.stringify(result)}`);
  let dexInstructions = result.instructions.filter((item) => item.name === 'transferChecked' && ((item.programId as any) === 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA') || 
                                                                                                (item.programId as any) === 'TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb')

    // logToFile(`dexInstructions========== ${JSON.stringify(dexInstructions)}`);    
  let setFrom = false
  let swap: any = {}
