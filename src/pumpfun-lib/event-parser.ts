  ...
  parseEvent(txn: VersionedTransactionResponse | ParsedTransactionWithMeta) {
    try {
      ...
      if (
        ...
        const accountKeys = txn.version === 0 ? txn.transaction.message.staticAccountKeys : (txn.transaction.message as Message).accountKeys;
        const instructions = txn.version === 0 ? txn.transaction.message.compiledInstructions : (txn.transaction.message as Message).instructions
