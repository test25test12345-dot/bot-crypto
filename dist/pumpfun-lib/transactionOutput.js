...
function transactionOutput(txn) {
    try {
        ...
        const mintAccount = instruction.accounts.find((item) => {
            if (!item)
                return false;
        ...
        if (!mintAccount) {
            ...
            console.error('Available accounts:', instruction.accounts.map((a) => a?.name).join(', '));
            return null;
        }
        ...
        const userAccount = instruction.accounts.find((item) => {
            if (!item)
                return false;
